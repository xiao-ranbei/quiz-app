    ...
    <div className="py-8 md:py-12">
      <section className="text-center mb-10">
        ...
        <div className="mt-6 flex flex-wrap gap-3 justify-center">
          ...
          <Link
            to="/exam"
            className="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-100 rounded-lg text-sm font-medium border border-slate-700"
          >
            <span className="inline-flex items-center gap-2">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto">
        <div className="rounded-xl bg-slate-800/60 border border-slate-700 p-5">
          ...
          <div className="text-sm text-slate-400">题库题目</div>
        </div>
        <div className="rounded-xl bg-slate-800/60 border border-slate-700 p-5">
          <div className="text-3xl text-emerald-300 font-bold mb-1">
            {loading ? '—' : categories.length}
          </div>
          <div className="text-sm text-slate-400">题目分类</div>
        </div>
        <div className="rounded-xl bg-slate-800/60 border border-slate-700 p-5">
          <div className="text-3xl text-amber-300 font-bold mb-1 flex items-center gap-2">
            <ClipboardCheck className="w-7 h-7" />
          </div>
          <div className="text-sm text-slate-400">可在个人中心配置 AI 解析</div>
        </div>
      </section>

      {!loading && categories.length > 0 && (
        <section className="max-w-4xl mx-auto mt-10">
          <h2 className="text-lg font-semibold text-slate-200 mb-3">快速练习</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {categories.slice(0, 6).map((c) => (
              <Link
                key={c.id}
                to={`/practice?category=${c.id}`}
                className="rounded-lg bg-slate-800/60 border border-slate-700 p-4 text-slate-200 hover:border-brand-500 hover:text-white"
              >
                {c.name}
              </Link>
            ))}
